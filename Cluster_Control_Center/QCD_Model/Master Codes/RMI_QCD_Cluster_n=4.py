...
def XY_A_U_B_4(T):
    ...
    BM = E_measurements  # Number of independent measurements for the bootstrap analysis
    steps = 2 * tau * BM  # Number of times the program will run
    E = -8 * (N * N) - 4 * y_tilde * (N * N)  # Initial Value of Energy since all spins start pointed up at \theta_i = 0.0
    L = zeros([N, N], float)  # Generates the lattice where each entry is a value of \theta_i

    # JT = J / T  # The parameter J divided by T (temperature) Multiplying dE by this will potentially save time,
    # but if this is done, make sure to multiply dE by T again when doing E += dE/(N*N)

    print("N=", N, "; A-union-B (n=4) XY-Model at T=", T)
    ...
    for x in range(steps + 1):
        ...
        if neighbor < N:
            ...
            dE += cos(L_update - L[i, 0]) - cos(L[i, j] - L[i, 0])
        dE *= 4 * -J
        dE += 4 * y_tilde * (cos(theta_coefficient * L[i, j]) - cos(theta_coefficient * L_update))
...
def XY_Replica_4_E(T):
    ...
    for x in range(steps + 1):
        ...
        L4_update = random() * 2 * pi

        if j < boundary:
            L1_update = L2_update = L3_update = L4_update
        # Calculates change in energy that would occur if this spin was accepted
        dE = 0.0
        # Starts calculating the nearest neighbor sum at location L[ i-1 , j]
        neighbor = i - 1
        if neighbor > -1:
            ...
            dE += cos(L4_update - L4[neighbor, j]) - cos(L4[i, j] - L4[neighbor, j])
            ...
            dE += cos(L4_update - L4[N - 1, j]) - cos(L4[i, j] - L4[N - 1, j])
        ...
        if neighbor < N:
            ...
            dE += cos(L4_update - L4[0, j]) - cos(L4[i, j] - L4[0, j])
        ...
        if neighbor > -1:
            ...
            dE += cos(L4_update - L4[i, neighbor]) - cos(L4[i, j] - L4[i, neighbor])
            ...
            dE += cos(L4_update - L4[i, N - 1]) - cos(L4[i, j] - L4[i, N - 1])
        ...
        if neighbor < N:
            ...
            dE += cos(L4_update - L4[i, 0]) - cos(L4[i, j] - L4[i, 0])
        dE *= -J
        dE += y_tilde * (cos(theta_coefficient * L1[i, j]) - cos(theta_coefficient * L1_update)) + y_tilde * (cos(theta_coefficient * L2[i, j]) - cos(theta_coefficient * L2_update)) + y_tilde * (cos(theta_coefficient * L3[i, j]) - cos(theta_coefficient * L3_update)) + y_tilde * (cos(theta_coefficient * L4[i, j]) - cos(theta_coefficient * L4_update))
...
def vary_temps_RMI(T_min, T_max, T_step):
    ...
    cores = Pool()
    result1 = cores.map(XY_Replica_4_E, temps)
    cores.close()
    cores.join()

    cores = Pool()
    result2 = cores.map(XY_A_U_B_4, temps)
    cores.close()
    cores.join()
    ...
    replica_4 = array(result1)
    A_U_B_4 = array(result2)
    ...
    E_replica_4 = replica_4[:, 1]
    sigma_replica_4 = replica_4[:, 2]

    E_A_U_B_4 = A_U_B_4[:, 1]
    sigma_A_U_B_4 = A_U_B_4[:, 2]

    return T_plot, E_replica_4, sigma_replica_4, E_A_U_B_4, sigma_A_U_B_4, E_normal, sigma_normal
...
def RMI_calc(T_min, T_max, T_step, save_data='no'):
    ...
    if save_data == 'yes':
        ...
        savetxt('{6}/RMI XY; {0}; {1}, {2}, {3}, {4}, {5}, n=4.txt'.format(date, E_measurements, T_min, T_max, T_step, N_global, folder_name), Data,
                header='This data took {0:.3f} hours and was recorded on {1}. This was run on the PSU Coeus Cluster.'.format(
                    t_elapse, datetime.datetime.today()))
...
if __name__ == '__main__':
    ...
    T_min = float(sys.argv[3].split(',')[0])
    T_max = float(sys.argv[4].split(',')[0])
